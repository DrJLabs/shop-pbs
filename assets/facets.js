class FacetFiltersForm extends HTMLElement {
  ...
  static renderFilters(html, event) {
    ...
    facetsToRender.forEach((element) => {
      ...
      const tabIndexElements = newElement.querySelectorAll('.wt-collapse__trigger');
      tabIndexElements.forEach((el) => el.setAttribute('tabindex', '0'));

      const container = document.querySelector(`.js-filter[data-index="${element.dataset.index}"]`);
...
class FacetRemove extends HTMLElement {
  ...
  closeFilter(event) {
    ...
    const priceRanges = document.querySelectorAll('.f-price__inputs > .f-price__val');
    ...
    if (
      ...
      if (priceSlider) {
        const maxPrice = document.querySelectorAll('price-range > div')[0].getAttribute('data-max');
