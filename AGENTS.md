...
## Testing Guidelines
There is no automated test suite; rely on `shopify theme check` plus targeted manual QA. Run `shopify theme check` before every push. For UI changes, run Playwright smoke tests (set `BASE_URL` or `SHOP_URL`, then `npx playwright test tests/smoke`) and capture a visual confirmation using Playwright against the local preview (`http://127.0.0.1:9292`) by default; include the screenshot or video path in PR notes. When using the live-store dev theme preview, set `BASE_URL` to the URL returned by `scripts/shopify/preview-url.sh` and note the preview URL in results. Only switch to live preview when explicitly requested. After merges into `dev`, push the theme to the dev theme ID (`shop-pbs/dev`) using `shopify theme push --store pbtz1b-1b.myshopify.com --theme 145915084935` and record the resulting preview URL. When changing forms, links, or buttons, add or update a Playwright smoke test that exercises the affected UI. For dev-server validation, start `shopify theme dev`, confirm the local preview URL, and verify changes against `http://127.0.0.1:9292` (preferred for Playwright checks). If a fix does not appear, restart the dev server from the same working tree to clear hot reload state. Exercise cart drawer, checkout linkages, and theme editor settings impacted by your change. Record live preview URLs or screenshots for UI adjustments. Maintain backwards compatibility with existing section presets.
...
Environment files:
- Use `.env` for production store credentials.
- Use `env.dev` for development store credentials.
- Always source the correct environment file before Admin API or Shopify CLI actions.
- Never commit `.env`.
